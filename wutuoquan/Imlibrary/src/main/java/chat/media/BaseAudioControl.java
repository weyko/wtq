...
abstract public class BaseAudioControl<T> {
	...
	private int state;
	protected boolean isEarPhoneModeEnable = ChatUtil.getInstance().getVoiceIsEarMode(); // 是否是听筒模式
	...
	protected boolean startAudio(Playable playable,AudioControlListener audioControlListener,
	...
	Runnable playRunnable = new Runnable() {
		...
		public void run() {
			if (currentAudioPlayer == null) {
				LogUtil.d("playRunnable run when currentAudioPlayer == null");
				return;
			}
	...
	protected int getUserSettingAudioStreamType() {
		ShowUtil.log("weyko","isEarPhoneModeEnable="+isEarPhoneModeEnable);
		// 听筒模式/扬声器模式
		if (isEarPhoneModeEnable) {
