 ...
 */
public abstract class MsgViewHolderBase extends ImViewHolder {
    ...
    @Override
    protected final void refresh(int position,Object item) {
        message = (MessageBean) item;
        setHeadImageView();
        setNameTextView();
        setTimeTextView(position);
        setStatus();
        setOnClickListener();
    ...
    public void refreshCurrentItem(int position) {
        if (message != null) {
            refresh(position,message);
        }
    }
     ...
     * 设置时间显示
     */
    private void setTimeTextView(int position) {
        if (getAdapter().needShowTime(position,message)) {
        ...
        String text = TimeUtil.getTimeShowString(message.getMsgTime(), false);
    ...
    private void setHeadImageView() {
        ...
        if (isMiddleItem()) {
            ...
            String headUrl=isReceivedMessage()?message.getImUserBean().getAvatar(): UserInfoHelp.getInstance(IMClient.getInstance().getContext()).getAvarar();
    ...
    public void setNameTextView() {
        if (message.getSessionType()== SessionTypeEnum.GROUPCHAT&& isReceivedMessage() && !isMiddleItem()) {
            nameTextView.setVisibility(View.VISIBLE);
            nameTextView.setText(message.getImUserBean().getName());
        } else {
    ...
    public void setContent() {
        ...
        if (isMiddleItem()) {
            ...
            if (isReceivedMessage()) {
                ...
                contentContainer.setBackgroundResource(R.drawable.chatfrom_bg);
            } else {
                setGravity(bodyContainer, Gravity.RIGHT);
                contentContainer.setBackgroundResource(R.drawable.chatto_bg);
            }
        }
    }
}
